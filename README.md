...
- 底部四个栏目改为柔和交叉淡化，并加入轻微缩放；切换后仍保留每个栏目的滚动位置。
- 页面底色、天空蓝、薄荷绿和分隔线进一步提亮，底部导航阴影减轻。
- Android PDF 阅读页使用与主应用一致的天空蓝返回胶囊、白色圆角阅读画布和海洋蓝分页按钮；翻页时页面淡入。
- PDF 加载、不可翻页和打开失败状态使用明确的视觉与文字反馈。

## 1.1.2 PDF 启动修复

- 修复 Android 15 打开 PDF 阅读页时，系统栏控制器早于窗口内容初始化而导致应用退出的问题。
- 增加 Android 8 与 Android 15 双版本阅读页启动回归测试；附件异常时留在阅读页并显示提示。
...
当前视觉焕新版安装包：[下载 ARM64 APK](artifacts/trail-capsule-1.1.2-arm64.apk)，适用 Android 8.0 及以上 ARM64 手机；[校验值](artifacts/SHA256SUMS.txt)。
