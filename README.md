...
- 行程可关联多份资料；行程详情中的资料卡片保持统一宽度，首页支持左右滑动切换栏目。
- 导入附件时可选择分类；跳过时自动归入“其他”。时区选择器以“北京”显示中国标准时区。
...
- 时区和分类选择改用统一的圆角选项面板；北京作为中国时区的显示名称。
- 行程支持关联多份资料，保留旧版单资料记录的读取兼容。
- 导入附件时可选分类；跳过或关闭时归入“其他”。
- 首页支持左右滑动切换四个栏目；行程详情在没有关联资料时也保持固定宽度。

## 1.1.4 奶油海岸视觉更新

- 全局配色调整为奶油白底、明快橙色主操作、薰衣草紫栏目强调和薄荷绿状态点缀。
- 首页下一站卡片、快捷入口、底部胶囊导航、资料分类图标与空状态加入更活泼的色彩和柔和投影。
- 表单、附件选择和 PDF 阅读页同步更新颜色、圆角与系统栏底色，保持各页面风格一致。
- Android 启动图标背景与海岸路线配色同步调整。

## 1.1.5 弹窗、导航与升级签名

- 收件分类弹窗铺满屏幕宽度；底部 Tab 使用轻量图标/标签动效与半透明磨砂底，并融合主页渐变背景。
- Release 包固定使用与 1.1.3/1.1.4 相同的签名证书，避免后续版本覆盖安装时发生签名冲突。
- 使用 `./scripts/build_android_release.sh` 构建并生成带 SHA-256 校验值的 ARM64 APK。`.tooling/signing/current-release.keystore` 是本地私钥备份，不能删除或提交到 Git；更换构建机器时须安全迁移此文件。
- 1.1.0–1.1.2 曾使用另一张签名证书；这些版本升级到 1.1.3 或更新版本前仍需卸载旧包一次。1.1.3–1.1.5 证书一致，可直接覆盖安装。

## 1.1.6 磨砂质感与提示动效

- 上传成功等提示条改为轻透深色样式，并调整出现、消失过渡时长；错误提示使用单独图标和暖色提示色。
- 首页卡片、编辑面板和主要按钮增加柔和背景模糊与半透明层，保留清晰边框、阴影和文字对比度。
- 橙色主操作按钮统一使用白色文字，并让“收进胶囊”悬浮按钮采用相同的磨砂材质。
- Release 继续使用与 1.1.3–1.1.5 相同的签名证书；Android 8.0 及以上 ARM64 手机可覆盖安装。

## 1.1.7 磨砂与提示动画修正

- 卡片、按钮的磨砂模糊加强，调整半透明层与浅色边缘，参考系统弹窗的柔和玻璃质感。
- 上传和复制提示改为独立浮层动画，明确执行上滑、淡入、轻微缩放，并在自动消失时反向淡出下滑。
- 沿用稳定签名证书，可从 1.1.3–1.1.6 覆盖安装。

## 1.1.8 提示条文字样式修正

- 修复自绘提示条误继承 Flutter 浮层默认文字样式、出现黄色双下划线的问题。
- 浮层使用透明 Material 容器，提示文字明确不带下划线；保留进出场动画和磨砂背景。
- 延续稳定签名，可从 1.1.3–1.1.7 覆盖安装。

## 1.1.9 点击回弹动效

- 按钮、底部 Tab、可点击卡片、列表选项和分类标签统一增加轻微按下与松开回弹效果；滑动列表时及时解除按压状态。
- Android 内置 PDF 阅读页的返回与翻页按钮同步加入按压回弹。
- 延续稳定签名，可从 1.1.3–1.1.8 覆盖安装。

## 1.2.0 Tab 过渡动画优化

- 主页面切换改为分段 Fade Through：旧页面先淡出并轻移，新页面随后淡入归位，避免内容重叠和闪烁。
- 左右滑动加入实时位移、透明度和边缘阻尼反馈；未达到阈值时自然回弹，达到阈值时只切换相邻页面。
- 底部 Tab 使用单个共享选中胶囊，选中块连续移动，图标固定尺寸并使用短促淡化与轻弹；保留各 Tab 的滚动位置。
- 遵循系统“减少动态效果”设置；延续稳定签名，可从 1.1.3–1.1.9 覆盖安装。
...
当前安装包：[下载 ARM64 APK](artifacts/trail-capsule-1.2.0-arm64.apk)，适用 Android 8.0 及以上 ARM64 手机；[校验值](artifacts/SHA256SUMS.txt)。
